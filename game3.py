...
channelid = "518501089784627215"

async def startGame3(game3, client):
  global player1, player2, player3, player4

  #creating all the player's hands and sending them.
  if game3.Playercount == 4:

    player1 = hand()
    player1.owner = game3.Players[0]
    await player1.send_hand()

    player2 = hand()
    player2.owner = game3.Players[1]
    await player2.send_hand()

    player3 = hand()
    player3.owner = game3.Players[2]
    await player3.send_hand()

    player4 = hand()
    player4.owner = game3.Players[3]
    await player4.send_hand()

    await startRound(game3, client)

  else:
    await client.send_message(destination=client.channel_get(channelid), content="You need 4 people to start the game!")

async def startRound(game3, client):
  if player1.score < game3.max_score or player2.score < game3.max_score or player3.score < game3.max_score or player4.score < game3.max_score:
    await choose_dealer(game3)
    await send_bcard(client, game3)
  else:
    score = []
    ...
    if winner == player1.score:
      ...
      print("player4 won")
    await endgame(game3, client)

async def endgame(game3, client):
  await client.remove_roles(player1.owner, discord.utils.get(player1.owner.server.roles, name="game3"))
  await client.remove_roles(player2.owner, discord.utils.get(player2.owner.server.roles, name="game3"))
  await client.remove_roles(player3.owner, discord.utils.get(player3.owner.server.roles, name="game3"))
  await client.remove_roles(player4.owner, discord.utils.get(player4.owner.server.roles, name="game3"))
  await client.purge_from(channel=client.channel_get(channelid), limit=1000)

async def send_bcard(client, game3):
  global usedcards_b
  if len(usedcards_b) != 16:
    card = "Deck/B" + str(randint(1,16)) + ".png"
    if card not in usedcards_b:
      await client.send_file(destination=client.get_channel(channelid), fp=card, content=str(game3.dealer.name)+" is the card czar!")
      usedcards_b.append(card)
  else:
    usedcards_b = []
    send_bcard(client)

async def playcard3(message, client, game3, card):
...
async def choose_dealer(game3):
  dealer = randint(1,4)

  if dealer == 1:
    player1.is_dealer = True
    game3.dealer = player1.owner
  elif dealer == 2:
    player2.is_dealer = True
    game3.dealer = player2.owner
  elif dealer == 3:
    player3.is_dealer = True
    game3.dealer = player3.owner
  elif dealer == 4:
    player4.is_dealer = True
    game3.dealer = player4.owner

async def choose_card3(message, client, game3, card):
  print("yeehaw")
